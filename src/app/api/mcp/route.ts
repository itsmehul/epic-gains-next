...
const mcpHandler = createMcpHandler(
  ...
  },
  {
    ...
    instructions: [
      ...
      "When importing a follow-along video, match /workouts/import: call get_youtube_import_prompt first, apply that prompt to the video, then pass the extracted JSON (overview + sections + exact MM:SS starts) to import_full_workout with sourceVideoUrl. Do not convert clocks to seconds or invent videoEndTime. Do not assemble the session with other workout tools.",
      ...
      "The video clock is the source of truth. Copy the second on the timer/beep/overlay (00:50, 01:25), not a rounded :00/:30 grid. Interval pattern is for suggested_time only. One row per labelled move — do not duplicate a name across consecutive 30s slots. Do not web-search for timestamps unless the video has no usable cues. Chapter markers are STARTS (never coarse ranges like Warm-Up 0:00–5:00).",
      "Do not shrink or invent clip ends to satisfy a guessed interval. If import fails, correct start timestamps from the video.",
    ].join("\n"),
  },
);
