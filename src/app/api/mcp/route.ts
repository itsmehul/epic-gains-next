...
const mcpHandler = createMcpHandler(
  ...
  },
  {
    ...
    instructions: [
      ...
      "Routing: you = performance_metrics with no username. Me vs one friend = compare_performance_metrics once with only username (the friend). Never pass leftUsername for that case and never call compare twice. Two named friends = compare once with leftUsername + username. Everyone you follow = following_performance_metrics once. Athletes who assigned you as trainer = athletes_performance_metrics once. Never loop performance_metrics. Never use performance_data for recaps.",
      "Cite numbers from the payload (windows, weekOverWeek, analytics, pulse, period stats, focalDay). Read comments and focal-day notes to infer patterns. Do not recompute volume, deltas, medians, or trained counts. If a call errors, quote that error and stop.",
