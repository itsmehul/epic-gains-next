...
const mcpHandler = createMcpHandler(
  ...
  },
  {
    ...
    instructions: [
      ...
      "When importing a follow-along video, call get_youtube_import_prompt with the YouTube URL. Apply that prompt to the video to extract real moves and times — do not invent values from the prompt schema. Then call import_full_workout once. Do not assemble the session with other workout tools.",
      "Refuse the import (do not call import_full_workout) if playback or embedding is disabled, or if the video is Zumba/dance choreography without labelled known exercises. Tell the user why.",
      "The video is the source of truth. Prefer on-screen timers, beeps, and the interval grid. Do not web-search for timestamps, chapters, or transcripts unless the video has no usable timing cues. If chapters exist, each marker is a START (never coarse ranges like Warm-Up 0:00–5:00).",
      "Lock starts to the cadence (60s, 45/15, 40/20, 30s). One grid slot per exercise — do not merge two work intervals. Skip rest, water, intro, and preview, but keep an abutting timeline: each videoEndTime equals the next videoStartTime; last move ends at video duration. Times are seconds.",
      "Call import_full_workout with sourceVideoUrl at the root. For every exercise include metric_profile, muscle_group, key_muscles (1–6), suggested_sets (typically 1), and suggested_time for the work interval or suggested_reps. Use tags for sections such as warmup, hiit, cooldown.",
    ].join("\n"),
  },
);
