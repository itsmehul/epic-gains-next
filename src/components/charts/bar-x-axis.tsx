...
const BarXAxisInner = memo(function BarXAxisInner({
  ...
  const labelsToShow = useMemo(() => {
    ...
    const allLabels = data.map((d) => {
      ...
      // Center the label under the bar group, keep first/last ticks inside the chart.
      const x = Math.min(
        Math.max(bandX + bandWidth / 2 + margin.left, edgePad),
        Math.max(edgePad, width - edgePad),
      );
      return { label, x };
    });
