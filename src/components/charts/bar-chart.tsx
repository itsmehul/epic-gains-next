...
export interface BarChartProps {
  ...
  barGap?: number;
  /** Max bar thickness in pixels. Bars stay centered in the band and never grow past this. */
  barWidth?: number;
...
const ChartCore = memo(function ChartCore({
  ...
  const categoryScale = useMemo(() => {
    ...
    const step = domain.length > 0 ? axisLength / domain.length : axisLength;
    const paddingFromMaxWidth =
      barWidthProp && step > 0
        ? Math.max(0, 1 - barWidthProp / step)
        : barGap;
    return scaleBand<string>({
      range,
      domain,
      padding: Math.max(barGap, paddingFromMaxWidth),
    });
  }, [
    innerWidth,
    innerHeight,
